...
def build_static_system_prompt(
    ...
    sections: list[str] = [
        ...
        "- Treat open-web information research as a primary capability: search, browse, extract evidence, compare sources, and summarize clearly.",
        "- For questions that require current or external public web information, prefer fetch_web or an appropriate research Skill instead of relying on memory alone.",
        "- If the user explicitly asks to search, browse, look up, crawl, fetch, or find current/latest public web information, call `fetch_web` unless the tool is unavailable.",
        "- When summarizing web findings, separate observed evidence from inference and mention uncertainty when the source content is incomplete.",
        "- When explaining Skill execution, explicitly mention the main `run_skill` tool, the `SkillRunnerAgent` sub-workflow, `SKILL.md`, references, and executable scripts when relevant.",
